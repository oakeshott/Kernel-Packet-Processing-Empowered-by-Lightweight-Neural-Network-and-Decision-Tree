...
class ConvNet(nn.Module):
    def __init__(self, in_dim, out_dim, channel_sizes, activation=nn.ReLU):
        ...
        output_dim = get_output_dim(get_output_dim(self.in_dim, kernel_size=3, stride=1), kernel_size=2, stride=2)
        output_dim = get_output_dim(get_output_dim(output_dim, kernel_size=3, stride=1), kernel_size=2, stride=2)

        layer_list = [
                nn.Conv1d(in_channels=1, out_channels=channel_sizes[0], kernel_size=3, stride=1, bias=False),
                nn.MaxPool1d(kernel_size=2, stride=2),
                # nn.AvgPool1d(kernel_size=2, stride=2),
                activation(),
                nn.Conv1d(in_channels=channel_sizes[0], out_channels=channel_sizes[1], kernel_size=3, stride=1, bias=False),
                nn.MaxPool1d(kernel_size=2, stride=2),
                # nn.AvgPool1d(kernel_size=2, stride=2),
                activation(),
                nn.Flatten(),
                nn.Linear(output_dim*channel_sizes[1], out_dim, bias=False)
                ]

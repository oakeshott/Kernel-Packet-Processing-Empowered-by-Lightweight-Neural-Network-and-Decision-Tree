...
def get_logdir(fold, n_fold, max_depth, is_binary):
    if is_binary:
        return os.path.join(os.path.join('runs', 'dt/binary-classification'), 'fixed-maxdepth' + str(max_depth))
    else:
        return os.path.join(os.path.join('runs', 'dt/multi-classification'), 'fixed-maxdepth' + str(max_depth))
...
def train_dt():
    ...
    dt = sklearn.tree.DecisionTreeClassifier(max_depth=opt.max_depth, max_leaf_nodes=1000, random_state=0)

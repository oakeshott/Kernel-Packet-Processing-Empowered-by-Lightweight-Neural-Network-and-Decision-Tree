...
from learn import OurDataset, output_scores
...
#	parser.add_argument("--categoriesMapping", type=str, default="categories_mapping.json", help="mapping of attack categories; see parse.py")
    ...
    dt.argstypes = [POINTER(ct.c_longlong), POINTER(ct.c_longlong), POINTER(ct.c_longlong), POINTER(ct.c_longlong), POINTER(ct.c_longlong), POINTER(ct.c_longlong), c_size_t]
    ...
    for sample in tqdm(final_x_test):
        ...
        dt(sample, children_left_pointer, children_right_pointer, value_pointer, feature_pointer, threshold_pointer, class_indices, children_left.size)
    ...
    with open(f"{opt.model_path}/classification_report.json", "w") as f:
        json.dump(ret, f, ensure_ascii=False, indent=4, sort_keys=True, separators=(',', ': '))
    with open(f"{opt.model_path}/inference_time.tsv", "w") as f:
